...
export default function PokemonInfoPanel({ selectedPokemon, setSelectedPokemon }: Props) {
    ...
    // Fetch evolution chain based on selectedPokemon.id
    useEffect(() => {
    ...
    return (
        <>
            ...
            <div
                className={`
          fixed top-0 right-0 h-full w-200 bg-white shadow-2xl p-6
            ...
            >
                ...
                {selectedPokemon && (
                    <>
                        ...
                        <section className="mb-6">
                            ...
                            <ul className="flex flex-wrap gap-2">
                                {selectedPokemon.abilities.map((ability) => (
                                    <li
                                        key={ability}
                                        className="capitalize bg-indigo-200 text-indigo-800 rounded-md px-3 py-1 font-medium"
                                    >
                                        {ability.replace("-", " ")}
                                    </li>
                                ))}
                        ...
                        <section className="mb-6 grid grid-cols-2 gap-x-6 gap-y-2">
                            <div>
                                <h3 className="text-xl font-semibold border-b border-gray-300 pb-1 mb-1 text-black">
                                    Height
                                </h3>
                                <p className="text-gray-700">{(selectedPokemon.height / 10).toFixed(1)} m</p>
                            </div>
                            <div>
                                <h3 className="text-xl font-semibold border-b border-gray-300 pb-1 mb-1 text-black">
                                    Weight
                                </h3>
                                <p className="text-gray-700">{(selectedPokemon.weight / 10).toFixed(1)} kg</p>
                            </div>
                        </section>
